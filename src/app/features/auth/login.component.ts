    ...
    <div class="min-h-screen flex items-center justify-center bg-slate-950 p-4 antialiased text-slate-200">
      <div class="w-full max-w-md bg-slate-900 border border-slate-800 rounded-2xl p-8 shadow-2xl backdrop-blur-sm">
        ...
        <div class="text-center mb-8">
          ...
          <p class="text-slate-400 text-xs mt-1 uppercase tracking-wider font-semibold">Plataforma de Control Multi-Tenant</p>
        </div>
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="space-y-5">
          ...
          <p class="text-xs text-center text-slate-400 mt-5">
            ...
            <a routerLink="/register" class="text-indigo-400 hover:text-indigo-300 hover:underline font-semibold transition-colors">Regístrate aquí</a>
          </p>
        </form>
      </div>
    </div>
    ...
    this.authService
      ...
      .subscribe({
        ...
        error: (err: HttpErrorResponse) => {
          this.errorMessage = err.error?.detail ?? 'Credenciales incorrectas o servidor inaccesible.';
        },
      });
  }
}